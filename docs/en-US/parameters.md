...
| `inputDir` | String path | Input file directory. | For a run from scratch, only the `MHDEquilibrium` file is needed. For a continued run, the `MHDContinue` and `PICContinue` files are also needed. They come from the previous run's `outputDir/final`. | `const std::string inputDir = "/path/to/input";` |
...
| `PSITMAX` | Real number | Outermost toroidal magnetic flux divided by 2π (Wb/rad). |  | `const double PSITMAX = 18.868213504765762;` |
...
| `perp2dP` | Real number | Coefficient for second-order perpendicular PIC-pressure dissipation. |  | `const mhdReal perp2dP = 1.0e-6;` |
...
| `para4dP` | Real number | Coefficient for fourth-order parallel PIC-pressure dissipation. |  | `const mhdReal para4dP = 0.0;` |
...
| `<Species>Vb` | Real number | Cutoff velocity of the slowing-down distribution, normalized by `VA0`. |  | `const picReal BeamVb = 0.1;` |
| `<Species>DeltaV` | Real number | Cutoff width of the slowing-down distribution, normalized by `VA0`. |  | `const picReal BeamDeltaV = 0.1;` |
| `<Species>Lambda0` | Real number | Center of `Lambda` in the anisotropic distribution. `Lambda = mu*B0/E`. |  | `const picReal BeamLambda0 = 0.4;` |
| `<Species>DeltaLambda2` | Real number | Square of the `Lambda` width in the anisotropic distribution. `Lambda = mu*B0/E`. |  | `const picReal BeamDeltaLambda2 = 1.0 / (4.5 * 4.5);` |
